...
- `query_knowledge_base` (`risk_level=read_only`) (registered only when vector DB provider + embedder are available and no pipeline is provided; wraps the full `query_sync()` pipeline with reranking and LLM synthesis)
- `query_context` (`risk_level=read_only`) (registered when pipeline is provided; builds retrieval sources without synthesis)
- `execute_context` (`risk_level=read_only`) (registered when pipeline is provided; synthesizes answer from a ContextPackage)
- `query_and_synthesize` (`risk_level=read_only`) (registered when pipeline is provided; full retrieve + synthesize in one call)
