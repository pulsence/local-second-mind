...
def _get_query_cache(config: LSMConfig) -> QueryCache | None:
    ...
    enabled = bool(getattr(config.query, "enable_query_cache", False))
    if not enabled:
        return None
    key = id(config)
    cache = _QUERY_CACHES.get(key)
    if cache is None:
        ttl_seconds = int(getattr(config.query, "query_cache_ttl", 3600) or 3600)
        max_size = int(getattr(config.query, "query_cache_size", 100) or 100)
        cache = QueryCache(
...
def clear_query_caches(config: Optional[LSMConfig] = None) -> int:
    ...
    Args:
        config: Optional config instance. When provided, clears only that
            config's cache; otherwise clears all query caches.
