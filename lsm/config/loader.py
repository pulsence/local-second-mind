...
def build_ingest_config(raw: Dict[str, Any], config_path: Path) -> IngestConfig:
    ...
    config = IngestConfig(
        ...
        enable_section_summaries=bool(ingest_raw.get("enable_section_summaries", False)),
        enable_file_summaries=bool(ingest_raw.get("enable_file_summaries", False)),
...
def build_query_config(raw: Dict[str, Any]) -> QueryConfig:
    ...
    config = QueryConfig(
        ...
        retrieval_profile=query_section.get("retrieval_profile", "hybrid_rrf"),
        k_dense=int(query_section.get("k_dense", 100)),
        k_sparse=int(query_section.get("k_sparse", 100)),
        rrf_dense_weight=float(query_section.get("rrf_dense_weight", 0.7)),
        rrf_sparse_weight=float(query_section.get("rrf_sparse_weight", 0.3)),
...
def config_to_raw(config: LSMConfig) -> Dict[str, Any]:
    ...
    return {
        ...
        "query": {
            ...
            "retrieval_profile": config.query.retrieval_profile,
            "k_dense": config.query.k_dense,
            "k_sparse": config.query.k_sparse,
            "rrf_dense_weight": config.query.rrf_dense_weight,
            "rrf_sparse_weight": config.query.rrf_sparse_weight,
